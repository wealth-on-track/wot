...
import { ensureEngineFiles, files, CATEGORIES, FINAL_STATES, readJson, AGENTS, WORKFLOW, WIP_LIMITS, normalizeJobs } from './lib.mjs';
...
ok('agents_defined', AGENTS.join(',') === 'scout,executer,qa', AGENTS.join(','));
ok('workflow_defined', WORKFLOW.includes('executer_sync') && WORKFLOW.includes('qa_review'), WORKFLOW.join(' -> '));
...
ok('wip_limits',
  jobs.filter((j) => j.state === 'proposal').length <= WIP_LIMITS.scout &&
  jobs.filter((j) => ['executer_sync', 'execution'].includes(j.state)).length <= WIP_LIMITS.executer &&
  jobs.filter((j) => j.state === 'qa_review').length <= WIP_LIMITS.qa,
  `scout<=${WIP_LIMITS.scout}, executer<=${WIP_LIMITS.executer}, qa<=${WIP_LIMITS.qa}`
);
ok('final_states_only_for_completed', history.every((j) => [...FINAL_STATES].includes(j.state)), `history=${history.length}`);
...
for (const j of jobs.concat(history)) {
  ...
  try {
    ...
    if (j.state !== 'proposal' && names.length === 0) artifactsOk = false;
    if (jobs.some((live) => live.id === j.id && live.state === 'qa_review') && !names.includes('commit-message.txt')) commitArtifactOk = false;
  } catch {
    if (j.state !== 'proposal') artifactsOk = false;
  }
}
ok('artifacts_present', artifactsOk, 'non-proposal jobs have artifacts');
ok('build_outputs_present', commitArtifactOk, 'live qa_review jobs have commit-message artifact');
