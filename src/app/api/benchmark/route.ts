        ...
        if (needsCurrentDay) {
            ...
            if (period === '1D') {
                ...
                if (cached && cacheAge < CACHE_TTL) {
                    ...
                    console.log(`[Benchmark] Fetching real-time 1D data for ${symbol}`);
                    currentDayData = await fetchFromYahooFinance(symbol, today, now);
...
async function fetchFromYahooFinance(
    ...
    try {
        const url = `https://query2.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&period1=${Math.floor(start.getTime() / 1000)}&period2=${Math.floor(end.getTime() / 1000)}`;
